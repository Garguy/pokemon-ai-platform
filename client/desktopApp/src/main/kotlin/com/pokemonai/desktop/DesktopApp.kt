...
import com.pokemonai.android.ui.*
    ...
    NavHost(navController, startDestination = Route.SPLASH) {
        ...
        composable(Route.LOGIN) {
            LoginScreen(onLoginSuccess = {
                navController.navigate(Route.QUESTIONNAIRE) { popUpTo(Route.LOGIN) { inclusive = true } }
            })
        }
        composable(Route.QUESTIONNAIRE) {
            QuestionnaireScreen(onSubmitted = { navController.navigate(Route.PROCESSING) })
        }
        composable(Route.PROCESSING) {
            ProcessingScreen(onGenerated = {
                navController.navigate(Route.RESULT) { popUpTo(Route.PROCESSING) { inclusive = true } }
            })
        }
