...
import com.pokemonai.client.graphql.MyHistoryQuery
...
class ApolloRecommendationRepository(private val apolloClient: ApolloClient) : RecommendationRepository {
    ...
    override suspend fun getHistory(): List<Recommendation> {
        val response = apolloClient.query(MyHistoryQuery()).execute()
        return response.dataOrThrow().myHistory.map { it.toModel() }
    }
...
private fun MyHistoryQuery.MyHistory.toModel() = Recommendation(
    id = id,
    pokemon = PokemonSummary(
        id = pokemon.id,
        name = pokemon.name,
